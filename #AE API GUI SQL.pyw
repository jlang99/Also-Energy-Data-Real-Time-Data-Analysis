        ...
        if name != "CDIA":
            ...
            if metercomms > time_date_compare:
                ...
                if name == "Wellons":
                    ...
                    meterdatakWM = max(row[6] for row in meterdata if row [6] is not None)
    ...
    if timecheck.hour <= 14 and selected_days_ago == 0:
        ...
        for table in tables:
            ...
            if "INV" in table_name:
                ...
                SELECT [kW] 
                FROM [{table_name}] 
                WHERE [kW] NOT IN (0, 1) AND [Timestamp] >= DATEADD('h', 11, ?) AND [Timestamp] <= DATEADD('h', 15, ?)""", (formatted_date, formatted_date))
